...
# 兼容两种运行方式：
# 1) 作为包内模块被页面导入（from utils.schedule_import import ...）
# 2) 命令行直接运行（python utils/schedule_import.py <xlsx>）
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..'))

import pandas as pd  # noqa: E402

from config import SCHEDULE_PROCESS_NAMES  # noqa: E402
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("用法: python utils/schedule_import.py <xlsx路径>")
        sys.exit(1)

    file_path_arg = sys.argv[1]
    if not os.path.exists(file_path_arg):
        print(f"文件不存在: {file_path_arg}")
        sys.exit(1)

    plans_out, errors_out = parse_schedule_excel(file_path_arg)
    _print_result(plans_out, errors_out)

    # 非零退出码便于 QA 脚本断言
    sys.exit(0 if plans_out else 1)
